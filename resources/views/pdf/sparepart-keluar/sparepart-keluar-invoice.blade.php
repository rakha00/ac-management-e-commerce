...
<head>
    <meta charset="UTF-8">
    <title>Sparepart Keluar Invoice - PT Global Servis Int</title>
</head>
...
<body style="font-family: Arial, sans-serif; font-size: 11pt; margin: 0; padding: 30px 40px; color: #000;">
    ...
    <table style="width: 100%; border-collapse: collapse;">
        <tr>
            <td style="width: 60%; vertical-align: top;">
                ...
                @include('pdf.components.customer-info', ['konsumen' => $sparepartKeluar->konsumen])
            </td>

            <td style="width: 40%; vertical-align: top; padding-left: 20px;">
                @include('pdf.components.invoice-header', [
                    'title' => 'INVOICE',
                    'tanggal_transaksi' => $sparepartKeluar->tanggal_keluar,
                    'nomor_invoice' => $sparepartKeluar->nomor_invoice,
                ])
            </td>
        </tr>
    </table>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 10pt;">
        ...
        <tbody>
            @php
                $maxItems = 14;
                $totalItems = $sparepartKeluar->detailSparepartKeluar->count();
                ...
                $subTotal = $sparepartKeluar->detailSparepartKeluar->sum(function ($item) {
            ...
            @foreach ($sparepartKeluar->detailSparepartKeluar->take($maxItems) as $item)
                @php
                    $currentItemNumber++;
                @endphp
                <tr>
                    ...
                    <td
                        ...
                        {{ $item->sparepart->nama_sparepart ?? 'N/A' }}
                    </td>
                    <td
                        style="padding: 8px; border-left: 1px solid #0072bc; border-right: 1px solid #0072bc; vertical-align: top; text-align: center; border-bottom: {{ $currentItemNumber == $maxItems ? '1px solid #0072bc' : 'none' }}">
                        {{ $item->jumlah_keluar }}
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 10pt;">
            ...
            <tbody>
                @php
                    ...
                    $pageItems = $sparepartKeluar->detailSparepartKeluar->skip($pageStart)->take($maxItems);
                @endphp
