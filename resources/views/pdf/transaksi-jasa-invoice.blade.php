...
<head>
    <title>Invoice Transaksi Jasa</title>
    <style>
        body {
...
<body>
    <div class="container">
            ...
            <tr>
                ...
                <td class="no-border" style="width: 40%;">
                    <table class="invoice-details">
                        ...
                        <tr>
                            <td colspan="2">{{ $transaksiJasa->konsumen->nama }}</td>
                        </tr>
                        <tr>
                            <td colspan="2">{{ $transaksiJasa->konsumen->alamat }}</td>
                        </tr>
                        <tr>
                            <td colspan="2">{{ $transaksiJasa->konsumen->telepon }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <p style="font-weight: bold;">NO. {{ $transaksiJasa->nomor_invoice_jasa }}</p>

        <table class="product-table">
            <thead>
                <tr>
                    <th style="width: 10%;">Banyaknya</th>
                    <th style="width: 50%;">Jenis Jasa</th>
            ...
            <tbody>
                @php
                    $totalPenjualan = 0;
                @endphp
                @foreach($transaksiJasa->detailTransaksiJasa as $detail)
                    <tr>
                        <td class="text-center">{{ $detail->qty }}</td>
                        <td>{{ $detail->jenis_jasa }}</td>
                        <td class="text-right">Rp {{ number_format($detail->harga_jasa, 0, ',', '.') }}</td>
                        <td class="text-right">Rp
                            {{ number_format($detail->harga_jasa * $detail->qty, 0, ',', '.') }}
                        </td>
                    </tr>
                    @php
                        $totalPenjualan += $detail->harga_jasa * $detail->qty;
                    @endphp
                @endforeach
            </tbody>
            <tfoot>
