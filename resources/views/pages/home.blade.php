                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4">
                        @foreach ($popularProducts as $product)
                            <x-product-card :id="$product->id" category="{{ $product->tipeAC->tipe_ac ?? 'AC' }}"
                                ...
                                image="{{ !empty($product->path_foto_produk) && is_array($product->path_foto_produk) && count($product->path_foto_produk) > 0 ? asset('storage/' . $product->path_foto_produk[0]) : asset('img/GSI.png') }}"
